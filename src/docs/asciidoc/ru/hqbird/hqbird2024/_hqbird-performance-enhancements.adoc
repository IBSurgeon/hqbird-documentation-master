...
* если `счётчик >= WarnCountAttachments и < MaxCountAttachments`, то коннект разрешается, но в `firebird.log` помещается сообщение вида
+
----
Database XXX: number of attachments is above warning level (N >= W)
----

* если `счётчик >= MaxCountAttachments`, то коннект не разрешается, клиенту выдаётся сообщение
+
----
