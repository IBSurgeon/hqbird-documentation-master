...
If you have enough space in the folder with the database, *just leave the path empty*, and click btn:[Ok], and replica will be created near the database. Or, you can specify other destination on the local drives with enough free space.
...
If you click btn:[Ок], HQbird will start the process of replica creation. There will be an appropriate message about it:
