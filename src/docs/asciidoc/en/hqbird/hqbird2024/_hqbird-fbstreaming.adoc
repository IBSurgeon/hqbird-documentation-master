...
The `fb_streaming` service (daemon) is used to process events. The `fb_streaming` service is included in the standard HQBird 2024 R2 distribution. The service monitors new replication log files, analyzes them, and generates events that are processed by one of the plugins.

Available plugins:

* `kafka_cdc_plugin` -- publishing CDC (Change Data Capture) events to Kafka (included in the HQBird 2024 R2 distribution);
* `rabbitmq_plugin` -- publishing replication events to RabbitMQ;
* `mongodb_events_plugin` -- storing replication events in MongoDB;
* `fts_lucene_plugin` -- automatic updating of full-text indexes without triggers (IBSurgeon Full Text Search UDR);
* `simple_json_plugin` -- saving replication logs in JSON format (included in the HQBird 2024 R2 distribution).

Plugins not included in the distribution are available upon request.
...
================================================================================
 ...
 Copyright (c) 2025 IBSurgeon ltd.
================================================================================
